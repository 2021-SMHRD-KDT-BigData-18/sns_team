	...
	<div id="backgroundArea">
		<div id="leftPage">
			<div class="navbar">
				<a class="nav-link active" aria-current="page" href="goMain.do"><img
					class="img" src="./image/로고새싹누끼.png" width="300"></a>
		...
		<div id="chatWrap" class=" border-start border-end border-1">
			...
			<div id="chatLog">
				...
				<div class="myMsg">
					<span><%=val.getCHAT_DT().substring(5) %></span> <span class="msg"><%=val.getCHAT_CONTENT() %></span>
				</div>
				<%} else{%>
				<div class="anotherMsg">
					<span class="anotherName"><%=val.getTALKER() %></span> <span
						class="msg"><%=val.getCHAT_CONTENT() %></span> <span><%=val.getCHAT_DT().substring(5) %></span>
				</div>
				<%} %>
				<%} %>
			...
			<div id="chatForm">
				<input id="inputMessage" type="text" autocomplete="off" size="30"
					onkeyup="enterkey()" placeholder="메시지를 입력하세요"> <input
					class="send_btn" type="submit" value="send" onclick="send()" />
			</div>
		</div>
				...
				<div class="card-body">
					...
					<c:if test="${sessionScope.user.getU_ID() != null}">
						<a href="logout.do" class="btn btn-primary"
							style="margin-left: 8%">로그아웃</a>
					</c:if>
					<c:if test="${sessionScope.user.getU_ID() == null}">
						<a href="goLogin.do" class="btn btn-primary"
							style="margin-left: 8%">로그인</a>
					</c:if>
				</div>
			</div>
    ...
    */};
    ...
    function send() {
        if (inputMessage.value == "") {
        	...
        	console.log(date);
        	let dString = month + '-' + day + ' '+date.getHours()+':'+date.getMinutes()+':'+date.getSeconds();

        webSocket.send($("#chat_id").val() + ",!," + inputMessage.value + ",!,"+ dString);
        }
        inputMessage.value = "";
    };
    //     엔터키를 통해 send함
